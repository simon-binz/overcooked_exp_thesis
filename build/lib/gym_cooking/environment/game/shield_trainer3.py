...
class Game:
    ...
    def __init__(self, env, num_humans, llas, ai_policies,  max_steps=100, render=False, idx_human=0):
        self._running = True
        self.env = env
        ...
        self.failure_dict = {'worlds': [], 'agent location': [], 'low level actions': []}
    ...
    def ai_only_event(self):
        ...
        for idx, agent in enumerate(self.env.unwrapped.world.agents):
            if idx >= self.num_humans:
                low_level_action = self.llas.pop(0)
                ...
                agent_pos = tuple(self.last_obs['player_0']['agent_location'])
                self.failure_dict['agent location'].append(agent_pos)
        ...
        agent_pos = tuple(observations['player_0']['agent_location'])
        self.check_failure(agent_pos)
        if all(dones.values()):
            self._running = False
    ...
    def check_failure(self, agent_pos):
        if agent_pos == self.carpet_pos:
            print('failure')
            self._running = False
