...
class Game:
    ...
    def __init__(self, env, num_humans, hlas, llas, ai_policies,  max_steps=100, render=False, idx_human=0):
        self._running = True
        self.env = env
    ...
    def ai_only_event(self):
        ...
        for idx, agent in enumerate(self.env.unwrapped.world.agents):
            if idx >= self.num_humans:
                if len(self.llas) == 0:
                    self.check_failure(0, action_mapping.Actions.CHOP_CARROT)
                    break
                else:
                    low_level_action = self.llas.pop(0)
                high_level_action = self.hlas.pop(0)
