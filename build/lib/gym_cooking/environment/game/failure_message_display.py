...
class GraphicPipeline:
    ...
    def __init__(self, env, hlas = None, display=False):
        self.env = env
        ...
        self.agent_button = Button((0, 0), image='agent-blue')
        ...
        self.general_text_rect.center = (self.excess_width + self.graphics_properties.width_pixel ,
                                           self.excess_height+ self.PIXEL_PER_TILE*0.5)
        self.texts = []
        self.rects = []
        for i in range(len(hlas)):
            text_message = HIGH_LEVEL_ACTION_IMAGE_MAP[hlas[i]]
            if i == len(hlas)-1:
                ...
                text= self.font.render(str(i+1) + ": " +text_message, True, Color.BLACK,
                                 Color.WHITE)
            rect = text.get_rect()
            rect.center = (self.excess_width + self.graphics_properties.width_pixel ,
        ...
        text_rect.center = (self.excess_width + self.graphics_properties.width_pixel*0.5+self.PIXEL_PER_TILE,
                                           self.excess_height+ self.PIXEL_PER_TILE*(1+(len(self.texts)+1)))
        self.texts.append(text)
