...
DISPLAY_TEXT_MAP = {"SliceTomato1": "SliceTomato",
                    "TomatoPlate1": "TomatoPlate",
                    "GiveTomato1": "GiveTomato",
                    "GiveChoppedTomato1": "GiveChoppedTomato",
                    "SliceLettuce1": "SliceLettuce",
                    "LettucePlate1": "LettucePlate",
                    "GiveLettuce1": "GiveLettuce",
                    "GiveChoppedLettuce1": "GiveChoppedLettuce",
                    "delivery1": "delivery",
                    "SliceTomato2": "SliceTomato",
                    "TomatoPlate2": "TomatoPlate",
                    "GiveTomato2": "GiveTomato",
                    "GiveChoppedTomato2": "GiveChoppedTomato",
                    "SliceLettuce2": "SliceLettuce",
                    "LettucePlate2": "LettucePlate",
                    "GiveLettuce2": "GiveLettuce",
                    "GiveChoppedLettuce2": "GiveChoppedLettuce",
                    "delivery2": "delivery",
                    "insert after 1": "insert after",
                    "insert before 1": "insert before",
                    "replace 1": "replace",
                    "forbid 1": "forbid",
                    "insert after 2": "insert after",
                    "insert before 2": "insert before",
                    "replace 2": "replace",
                    "forbid 2": "forbid"
                    }
...
class GraphicPipeline:
    ...
    def __init__(self, env, world_dict, hlas, display=False):
        ...
        for i, key in enumerate(hlas):
            ...
            if len(hlas)>5:
                button = Button((-0.75*self.excess_width + (i+1)*self.PIXEL_PER_TILE, -0.5*self.excess_height+(2)*self.PIXEL_PER_TILE+self.graphics_properties.height_pixel), image=text, text=text)
            else:
                button = Button((-0.5 * self.excess_width + (i+1) * self.PIXEL_PER_TILE,
        ...
        for i in range(len(world_dict['high level actions'][0])):
            text = HIGH_LEVEL_ACTION_IMAGE_MAP[world_dict['high level actions'][0][i]]
            if i == len(world_dict['high level actions'])-1:
                ...
                button = Button((-0.5*self.excess_width+ self.PIXEL_PER_TILE* (i+1), -0.5*self.excess_height), image= text, text = text)
            self.action_stack1.append(button)

        for i in range(len(world_dict['high level actions'][1])):
            text = HIGH_LEVEL_ACTION_IMAGE_MAP[world_dict['high level actions'][1][i]]
            if i == len(world_dict['high level actions'][1])-1:
                button = Button((-0.5 * self.excess_width + self.PIXEL_PER_TILE* (i+1),
                                 -0.5 * self.excess_height + self.PIXEL_PER_TILE*1.5 ), image=text, text=text, background_color=Color.RED)
            else:
                button = Button((-0.5*self.excess_width+ self.PIXEL_PER_TILE* (i+1), -0.5*self.excess_height+ self.PIXEL_PER_TILE*1.5 ), image= text, text = text)
            self.action_stack2.append(button)



        self.action_buttons = self.action_stack1 + self.action_stack2
        self.agent_button = Button((-0.5 * self.excess_width, -0.5*self.excess_height), image='agent-blue')
        self.agent_button2 = Button((-0.5 * self.excess_width, -0.5 * self.excess_height +self.PIXEL_PER_TILE*1.5), image='agent-magenta')

        self.agent_button3 = Button((-0.5 * self.excess_width, -0.5 * self.excess_height+ self.PIXEL_PER_TILE * 3.5), image='agent-blue')
        self.agent_button4 = Button((-0.5 * self.excess_width, -0.5 * self.excess_height + self.PIXEL_PER_TILE * 5),
                                    image='agent-magenta')

        self.agent_buttons = [self.agent_button,self.agent_button2,self.agent_button3,self.agent_button4]

        self.font = pygame.font.SysFont('timesnewroman', 30)
        self.small_font = pygame.font.SysFont('timesnewroman', 18)

        self.describe_text1 = self.font.render('1: Select which action needs to be modified to avoid the failure:',
                                               True,
        ...
        self.insert_text_rect1 = self.insert_text.get_rect()
        self.insert_text_rect1.center = (0.5 * self.excess_width + self.PIXEL_PER_TILE * 1.5,
                                         0.5 * self.excess_height + self.graphics_properties.height_pixel +
                                         self.PIXEL_PER_TILE / 4-self.PIXEL_PER_TILE*1.5)

        self.insert_text_rect2 = self.insert_text.get_rect()
        self.insert_text_rect2.center = (0.5 * self.excess_width + self.PIXEL_PER_TILE * 1.5,
        ...
        self.a11_text_rect = self.action_text.get_rect()
        self.a11_text_rect.center = (0.5 * self.excess_width + self.PIXEL_PER_TILE * 1.5,
                                     0.5 * self.excess_height + self.graphics_properties.height_pixel + self.PIXEL_PER_TILE / 2-self.PIXEL_PER_TILE*1.5)

        self.a21_text_rect = self.action_text.get_rect()
        self.a21_text_rect.center = (0.5 * self.excess_width + self.PIXEL_PER_TILE * 1.5,
        ...
        self.before_text_rect1 = self.before_text.get_rect()
        self.before_text_rect1.center = (0.5 * self.excess_width + self.PIXEL_PER_TILE * 1.5,
                                         0.5 * self.excess_height + self.graphics_properties.height_pixel - self.PIXEL_PER_TILE * (
                                                 3 / 4))

        self.before_text_rect2 = self.before_text.get_rect()
        self.before_text_rect2.center = (0.5 * self.excess_width + self.PIXEL_PER_TILE * 1.5,
                                         0.5 * self.excess_height + self.graphics_properties.height_pixel + self.PIXEL_PER_TILE * (
                                                    3 / 4))

        self.insert_button1 = Button((-0.5 * self.excess_width + self.PIXEL_PER_TILE,
                                      -0.5 * self.excess_height + self.graphics_properties.height_pixel- self.PIXEL_PER_TILE*1.5),
                                     text='insert before 1')

        self.insert_button2 = Button((-0.5 * self.excess_width + self.PIXEL_PER_TILE,
                                      -0.5 * self.excess_height + self.graphics_properties.height_pixel),
                                     text='insert before 2')
        ...
        self.inserta_text_rect1 = self.inserta_text.get_rect()
        self.inserta_text_rect1.center = (0.5 * self.excess_width + self.PIXEL_PER_TILE * 2.5,
                                          0.5 * self.excess_height + self.graphics_properties.height_pixel +
                                          self.PIXEL_PER_TILE / 4 -self.PIXEL_PER_TILE*1.5)


        self.inserta_text_rect2 = self.inserta_text.get_rect()
        self.inserta_text_rect2.center = (0.5 * self.excess_width + self.PIXEL_PER_TILE * 2.5,
                                          0.5 * self.excess_height + self.graphics_properties.height_pixel + self.PIXEL_PER_TILE / 4)

        self.a12_text_rect = self.action_text.get_rect()
        self.a12_text_rect.center = (0.5 * self.excess_width + self.PIXEL_PER_TILE * 2.5,
                                     0.5 * self.excess_height + self.graphics_properties.height_pixel + self.PIXEL_PER_TILE / 2- self.PIXEL_PER_TILE*1.5)

        self.a22_text_rect = self.action_text.get_rect()
        self.a22_text_rect.center = (0.5 * self.excess_width + self.PIXEL_PER_TILE * 2.5,
                                     0.5 * self.excess_height + self.graphics_properties.height_pixel + self.PIXEL_PER_TILE / 2)
        ...
        self.after_text_rect1 = self.after_text.get_rect()
        self.after_text_rect1.center = (0.5 * self.excess_width + self.PIXEL_PER_TILE * 2.5,
                                        0.5 * self.excess_height + self.graphics_properties.height_pixel - self.PIXEL_PER_TILE * (
                                                3 / 4))

        self.after_text_rect2 = self.after_text.get_rect()
        self.after_text_rect2.center = (0.5 * self.excess_width + self.PIXEL_PER_TILE * 2.5,
                                        0.5 * self.excess_height + self.graphics_properties.height_pixel + self.PIXEL_PER_TILE * (
                                                   3 / 4))
        self.inserta_button1 = Button((-0.5 * self.excess_width + self.PIXEL_PER_TILE * 2,
                                       -0.5 * self.excess_height + self.graphics_properties.height_pixel-self.PIXEL_PER_TILE*1.5),
                                      text='insert after 1')

        self.inserta_button2 = Button((-0.5 * self.excess_width + self.PIXEL_PER_TILE * 2,
                                       -0.5 * self.excess_height + self.graphics_properties.height_pixel),
                                      text='insert after 2')
        ...
        self.replace_text_rect1 = self.replace_text.get_rect()
        self.replace_text_rect1.center = (0.5 * self.excess_width + self.PIXEL_PER_TILE * 3.5,
                                          0.5 * self.excess_height + self.graphics_properties.height_pixel + self.PIXEL_PER_TILE *1/3  -self.PIXEL_PER_TILE*1.5)

        self.replace_text_rect2 = self.replace_text.get_rect()
        self.replace_text_rect2.center = (0.5 * self.excess_width + self.PIXEL_PER_TILE * 3.5,
                                          0.5 * self.excess_height + self.graphics_properties.height_pixel + self.PIXEL_PER_TILE *1/3)

        self.a13_text_rect = self.action_text2.get_rect()
        self.a13_text_rect.center = (0.5 * self.excess_width + self.PIXEL_PER_TILE * 3.5,
                                     0.5 * self.excess_height + self.graphics_properties.height_pixel + self.PIXEL_PER_TILE * 2/3 -self.PIXEL_PER_TILE*1.5)

        self.a23_text_rect = self.action_text2.get_rect()
        self.a23_text_rect.center = (0.5 * self.excess_width + self.PIXEL_PER_TILE * 3.5,
                                     0.5 * self.excess_height + self.graphics_properties.height_pixel + self.PIXEL_PER_TILE * 2/3)

        self.replace_button1 = Button((-0.5 * self.excess_width + self.PIXEL_PER_TILE * 3,
                                       -0.5 * self.excess_height + self.graphics_properties.height_pixel- self.PIXEL_PER_TILE*1.5),
                                      text='replace 1')

        self.replace_button2 = Button((-0.5 * self.excess_width + self.PIXEL_PER_TILE * 3,
                                       -0.5 * self.excess_height + self.graphics_properties.height_pixel),
                                      text='replace 2')
        ...
        self.forbid_text_rect1 = self.forbid_text.get_rect()
        self.forbid_text_rect1.center = (0.5 * self.excess_width + self.PIXEL_PER_TILE * 4.5,
                                         0.5 * self.excess_height + self.graphics_properties.height_pixel + self.PIXEL_PER_TILE *1/3 -self.PIXEL_PER_TILE*1.5)

        self.forbid_text_rect2 = self.forbid_text.get_rect()
        self.forbid_text_rect2.center = (0.5 * self.excess_width + self.PIXEL_PER_TILE * 4.5,
                                         0.5 * self.excess_height + self.graphics_properties.height_pixel + self.PIXEL_PER_TILE *1/3)

        self.a14_text_rect = self.action_text2.get_rect()
        self.a14_text_rect.center = (0.5 * self.excess_width + self.PIXEL_PER_TILE * 4.5,
                                     0.5 * self.excess_height + self.graphics_properties.height_pixel + self.PIXEL_PER_TILE * 2/3-self.PIXEL_PER_TILE*1.5)

        self.a24_text_rect = self.action_text2.get_rect()
        self.a24_text_rect.center = (0.5 * self.excess_width + self.PIXEL_PER_TILE * 4.5,
                                     0.5 * self.excess_height + self.graphics_properties.height_pixel + self.PIXEL_PER_TILE * 2 / 3)

        self.forbid_button1 = Button((-0.5 * self.excess_width + self.PIXEL_PER_TILE * 4,
                                      -0.5 * self.excess_height + self.graphics_properties.height_pixel- self.PIXEL_PER_TILE*1.5), text='forbid 1')
        self.forbid_button2 = Button((-0.5 * self.excess_width + self.PIXEL_PER_TILE * 4,
                                      -0.5 * self.excess_height + self.graphics_properties.height_pixel), text='forbid 2')

        self.shields_buttons = [self.insert_button1, self.inserta_button1, self.replace_button1, self.forbid_button1,
                                self.insert_button2, self.inserta_button2, self.replace_button2, self.forbid_button2]
        self.shields_texts_w = [self.insert_text, self.before_text, self.inserta_text, self.after_text, self.replace_text,
                              ...
                              self.insert_text, self.before_text, self.inserta_text, self.after_text, self.replace_text,
                              self.forbid_text, self.action_text, self.action_text, self.action_text2, self.action_text2]
        self.shields_texts_g = [self.insert_text_g, self.before_text_g, self.inserta_text_g, self.after_text_g, self.replace_text_g,
                              self.forbid_text_g, self.action_text_g, self.action_text_g, self.action_text2_g, self.action_text2_g,
                              self.insert_text_g, self.before_text_g, self.inserta_text_g, self.after_text_g, self.replace_text_g,
                              self.forbid_text_g, self.action_text_g, self.action_text_g, self.action_text2_g, self.action_text2_g]
        self.shields_texts = self.shields_texts_w

        self.shields_texts_rects = [self.insert_text_rect1, self.before_text_rect1, self.inserta_text_rect1,
                                    self.after_text_rect1, self.replace_text_rect1, self.forbid_text_rect1,
                                    self.a11_text_rect, self.a12_text_rect, self.a13_text_rect, self.a14_text_rect,
                                    self.insert_text_rect2, self.before_text_rect2, self.inserta_text_rect2,
                                    self.after_text_rect2, self.replace_text_rect2, self.forbid_text_rect2,
                                    self.a21_text_rect, self.a22_text_rect, self.a23_text_rect, self.a24_text_rect]


        #possibly add confirm
        self.confirm_text = self.small_font.render('Confirm', True, Color.BLACK, Color.WHITE)
        ...
        for i in range(2):
            button = Button((
                            self.excess_width * 0.5 + self.graphics_properties.width_pixel * 0.5 + self.PIXEL_PER_TILE * (i+1),
                            self.PIXEL_PER_TILE),
    ...
    def draw_action_selection_buttons(self):
        ...
        if self.action_selection_mode:
            if DISPLAY_TEXT_MAP[text] == "forbid":
                message_text= self.font.render("3: Select an action to " + DISPLAY_TEXT_MAP[text] + " to avoid the failure:", True, Color.BLACK, Color.WHITE)
            else:
                message_text = self.font.render("3: Select an action to " + DISPLAY_TEXT_MAP[text] + " " + text2 + " to avoid the failure:", True,
                                                Color.BLACK, Color.WHITE)
    ...
    def highlight_clickable_buttons(self):
        if not self.shield_selection_mode:
            s = pygame.Surface((self.PIXEL_PER_TILE * len(self.shields_buttons)/2, self.PIXEL_PER_TILE))
            s.set_alpha(128)
            s.fill(Color.GREY)
            self.screen.blit(s, (self.insert_button1.loc[0]+self.excess_width,self.insert_button1.loc[1]+self.excess_height))
            self.screen.blit(s, (
            self.insert_button1.loc[0] + self.excess_width, self.insert_button1.loc[1] + self.excess_height + self.PIXEL_PER_TILE*1.5))
        if not self.action_selection_mode:
