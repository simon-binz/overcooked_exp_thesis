...
class GraphicPipeline:
    ...
    def __init__(self, env, action_history, carpet = False, display=False):
        self.env = env
        ...
        for actions in action_history:
            self.hlas.append(actions[0])
        self.action_stack_buttons = []
        agent_button = Button((-self.PIXEL_PER_TILE, -2 * self.PIXEL_PER_TILE),
                              image="agent-blue", color=Color.BLACK)

        self.action_stack_buttons.append(agent_button)
        for i in range(len(self.hlas)):
            if action_history[i][1]== 'failure':
                button = Button((self.PIXEL_PER_TILE * i, -2 * self.PIXEL_PER_TILE),
                                image=HIGH_LEVEL_ACTION_IMAGE_MAP[self.hlas[i]], text=self.hlas[i],
                                color=Color.BLACK, background_color= Color.RED)
            else:
                button = Button((self.PIXEL_PER_TILE * i, -2 * self.PIXEL_PER_TILE),
                                image=HIGH_LEVEL_ACTION_IMAGE_MAP[self.hlas[i]], text=self.hlas[i],
                                color=Color.BLACK)
            self.action_stack_buttons.append(button)

        self.floor = []
        for i in range((self.graphics_properties.width_pixel//self.PIXEL_PER_TILE-2)*(self.graphics_properties.height_pixel//self.PIXEL_PER_TILE-2)):
            j=0
            button = Button(((self.PIXEL_PER_TILE*(i+1)), self.PIXEL_PER_TILE*(j+1)), color= Color.FLOOR, background_color=Color.FLOOR)
            if (i + 1) %(self.graphics_properties.width_pixel - 2) == 0:
                j += 1
    ...
    def on_render(self):
        ...
        for button in self.action_stack_buttons:
            if button.text == self.hla:
    ...
    def draw_static_object(self, static_object: StaticObject):
        ...
        fill = pygame.Rect(sl[0] + self.excess_width, sl[1]+ self.excess_height, self.graphics_properties.pixel_per_tile,
