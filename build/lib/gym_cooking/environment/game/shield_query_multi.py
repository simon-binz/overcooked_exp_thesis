...
from gym_cooking.environment.game import shield_display_multi
...
class Game:
    ...
    def __init__(self, env, num_humans, world_dict, hlas, ai_policies, max_steps=100, render=False, idx_human=0):
        ...
        # Visual parameters
        self.graphics_pipeline = shield_display_multi.GraphicPipeline(env, world_dict, hlas, self.render)
    ...
    def checkForButtonClicks(self, event):
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            if (self.graphics_pipeline.isValidButton(x, y)):
                ...
                for i in range(len(self.graphics_pipeline.action_stack1)):
                    if button.text == 'Abort':
                        self._running = False
                        self.aborted = True
                    if button is self.graphics_pipeline.action_stack1[i]:
                        self.env.unwrapped.world = self.world_dict['worlds'][i]
                        self.on_render()
                for i in range(len(self.graphics_pipeline.action_stack2)):
                    if button.text == 'Abort':
                        self._running = False
                        self.aborted = True
                    if button is self.graphics_pipeline.action_stack2[i]:
                        self.env.unwrapped.world = self.world_dict['worlds'][i]
                        self.on_render()
    ...
    def get_shield(self):
        for button in self.graphics_pipeline.action_buttons:
            if button.is_clicked:
                failure = button.text
                if button in self.graphics_pipeline.action_stack1:
                    failure += "1"
                else:
                    failure += "2"
