...
from gym_cooking.environment.game import shield_display3
...
class Game:
    ...
    def __init__(self, env, num_humans, world_dict, llas, ai_policies, max_steps=100, render=False, idx_human=0, carpet = False):
        self._running = True
        self.env = env
        self.play = bool(num_humans)
        self.render = render or self.play
        # Visual parameters
        self.graphics_pipeline = shield_display3.GraphicPipeline(env, world_dict, llas,carpet ,self.render)
    ...
    def checkForButtonClicks(self, event):
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            if (self.graphics_pipeline.isValidButton(x, y)):
                ...
                #end if
                if button in self.graphics_pipeline.action_selection_buttons:
                    self._running = False
    ...
    def get_shield(self):
        ...
        return {'Failure': failure, 'Shield type': shield_type, 'Shield action': shield_action,
                'agent location': self.env.unwrapped.world.agents[0].location}
